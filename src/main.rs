...
fn main() -> eframe::Result<()>{
    ...
    let options = eframe::NativeOptions {
        vsync: false,
        viewport: egui::ViewportBuilder::default().with_decorations(false).with_transparent(true).with_has_shadow(false).with_inner_size([320.0,320.0]).with_movable_by_background(true).with_window_level(egui::WindowLevel::AlwaysOnTop).with_mouse_passthrough(false),
        ..Default::default()
    };
...
impl Default for AtApp<'_> {
    fn default() -> Self {
        Self { uimode: false, id: "".to_owned(), ids: vec![], path: egui::include_image!("../assets/workplease.webp"), ws: "".to_owned(), speed:0}
    }
}
        ...
        egui::CentralPanel::default().frame(Frame::NONE).show(ctx, |ui| {
            if self.uimode{
                ...
                #[cfg(feature = "steamcmd")]{
                    ...
                    ui.add(egui::Slider::new(&mut self.speed, 0..=240));
                });
